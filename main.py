    ...
    def proc_flightSegments(self):
        for flightlist in self.flightItineraryList:
            ...
            dellist = ['sequenceNo', 'marketAirlineCode',
                       'departureProvinceId', 'departureCityId', 'departureCityCode', 'departureAirportShortName',
                       'departureTerminal',
                       'arrivalProvinceId', 'arrivalCityId', 'arrivalCityCode', 'arrivalAirportShortName',
                       'arrivalTerminal',
                       'transferDuration', 'stopList', 'leakedVisaTagSwitch', 'trafficType', 'highLightPlaneNo',
                       'mealType',
                       'operateAirlineCode', 'arrivalDateTime', 'departureDateTime', 'operateFlightNo',
                       'operateAirlineName']
    ...
    def proc_priceList(self):
        for flightlist in self.flightItineraryList:
            ...
            price_info = {'flightNo': flightNo,
                          ...
                          'bussiness_origin': bussiness_origin, 'bussiness_low': bussiness_low,
    ...
    def mergedata(self):
        try:
            ...
            order = ['数据获取日期', '航班号', '航空公司',
                     '出发日期', '出发时间', '到达日期', '到达时间', '飞行时长', '出发国家', '出发城市', '出发机场',
                     '出发机场三字码',
            ...
            filename = os.getcwd() + '\\' + self.date + '\\' + self.date + '-' + self.city[0] + '-' + self.city[
                1] + '.csv'
